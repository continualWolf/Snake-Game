...
})({"9v79w":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "3e53729efb306b9f";
"use strict";
...
},{}],"6srC3":[function(require,module,exports) {
var _home = require("./Views/Home");
let home = new (0, _home.Home)("layout");
home.open();

},{"./Views/Home":"k8lHm"}],"k8lHm":[function(require,module,exports) {
...
class Home {
    ...
    #addEventListeners() {
        ...
        snakeGameBtn?.addEventListener("click", function handleClick(event) {
            let game = new (0, _game.Game)(divId);
            game.start();
        });
        const chessGameBtn = document.getElementById("openChessGame");
        chessGameBtn?.addEventListener("click", function handleClick(event) {
            let chess = new (0, _chess.Chess)(divId);
            chess.startGame();
        });
    }
}

},{"./Game":"jab1Z","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./Chess":"gWKzG"}],"jab1Z":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Game {
    ...
    start() {
        let grid = new (0, _grid.Grid)();
        grid.AddGrid(this.divId);
        let snake = new (0, _snake.Snake)();
        ...
        snake.addAppleToGrid([
            8,
            8
        ]);
        // the direction the snake starts in
        let snakeDirection = (0, _keys.Keys).D;
    ...
    #attachGameOverEventListeners() {
        ...
        exit?.addEventListener("click", function handleClick(event) {
            let home = new (0, _home.Home)(divId);
            home.open();
        });
    }
    gameOver() {
...
},{"../Constants/Keys":"liR58","../Grid":"3n5kP","../Snake":"1xsIh","./Home":"k8lHm","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"liR58":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}],"3n5kP":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Grid {
    AddGrid(elementId) {
        ...
        if (element) {
            let gridItems = ``;
            for(let i = 0; i < 361; i++)gridItems += `<div class="grid-item"></div>`;
            element.innerHTML = `<p class="gameScore">Score: <span id="mainGameScore">0</span></p><div class="grid-container">${gridItems}</div>`;
        }
    }
    AddChessGrid(elementId) {
        const element = document.getElementById(elementId);
        if (element) {
            let gridItems = ``;
            for(let i = 0; i < 64; i++)gridItems += `<div class="grid-item-chess"></div>`;
            element.innerHTML = `<div class="grid-container-chess">${gridItems}</div>`;
        }
    }
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1xsIh":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Snake {
    clearSnakeFromGrid() {
        ...
        gridItems.forEach((element)=>{
            element.style.background = element.style.background == "darkmagenta" ? i % 2 != 0 ? "#39a0cc" : "#48cfd9" : element.style.background;
            i++;
        });
    }
    addAppleToGrid(coord) {
        try {
            document.getElementsByClassName("grid-item")[this.getIndexOfItemUsingCoord(coord)].setAttribute("style", "background:red");
        } catch (err) {
    ...
    addSnakeToGrid(coords) {
        this.clearSnakeFromGrid();
        for(let i = 0; i < coords.length; i++){
            this.coords = coords;
            let gridItem = document.getElementsByClassName("grid-item")[this.getIndexOfItemUsingCoord(coords[i])];
            if (gridItem) gridItem.setAttribute("style", "background:darkmagenta");
        }
    }
    moveSnakeForward(direction) {
        ...
        for(let i = length - 1; i > -1; i--)if (i == length - 1) {
            tempValue = [
                this.coords[i][0],
                this.coords[i][1]
            ];
            this.coords[i] = this.GetNewCoordBasedOnDirection(this.coords[i], direction);
            let isNextSquareGreen = this.IsNextCoordOccupied(this.coords[i]);
            if (isNextSquareGreen) return true;
            let IsNextCoordApple = this.IsNextCoordApple(this.coords[i]);
            if (IsNextCoordApple) appleEaten = true;
        } else {
            let temp = [
                this.coords[i][0],
                this.coords[i][1]
        ...
        if (appleEaten) {
            let newCoords = this.GetNewCoordBasedOnDirection(this.coords[0], direction);
        ...
        if (appleEaten) {
            ...
            this.score += 1;
            let gameScore = document.getElementById("mainGameScore");
            gameScore.innerHTML = `${this.score}`;
        }
        return false;
    }
    GetNewCoordBasedOnDirection(coord, direction) {
        if (direction == (0, _keys.Keys).D) coord[0] += 1;
        else if (direction == (0, _keys.Keys).W) coord[1] += 1;
        else if (direction == (0, _keys.Keys).S) coord[1] -= 1;
        else if (direction == (0, _keys.Keys).A) coord[0] -= 1;
        return coord;
    }
    IsNextCoordOccupied(coord) {
        const coordSquare = document.getElementsByClassName("grid-item")[this.getIndexOfItemUsingCoord(coord)];
        let outOfBounds = coord[0] < 1 || coord[0] > this.gridDimension || coord[1] < 1 || coord[1] > this.gridDimension ? true : false;
        if (!coordSquare) return true;
        return coordSquare.style.background == "darkmagenta" || outOfBounds;
    }
    IsNextCoordApple(coord) {
        const coordSquare = document.getElementsByClassName("grid-item")[this.getIndexOfItemUsingCoord(coord)];
        if (!coordSquare) return false;
        return coordSquare.style.background == "red";
    }
    generateNewAppleLocation(min, max) {
    ...
    constructor(){
        this.coords = [];
        this.gridDimension = 19;
        this.score = 0;
        this.getIndexOfItemUsingCoord = (coord)=>coord[0] + this.gridDimension * (this.gridDimension - coord[1]) - 1;
    }
}

},{"./Constants/Keys":"liR58","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gWKzG":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Chess {
    constructor(divId){
        this.divId = divId;
        this.gridDimension = 8;
        this.getIndexOfItemUsingCoord = (coord)=>coord[0] + this.gridDimension * (this.gridDimension - coord[1]) - 1;
    }
    startGame() {
        let chessGrid = new (0, _grid.Grid)();
        chessGrid.AddChessGrid(this.divId);
        this.resetChessPieces();
    }
    resetChessPieces() {
        let gridItems = Array.from(document.getElementsByClassName("grid-item-chess"));
        let indexs = [
            this.getIndexOfItemUsingCoord([
                1,
                2
            ]),
            this.getIndexOfItemUsingCoord([
...
},{"../Grid":"3n5kP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9v79w","6srC3"], "6srC3", "parcelRequire94c2")

//# sourceMappingURL=index.fb306b9f.js.map
